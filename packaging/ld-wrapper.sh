...
if [ -n "$LD_LIBRARY_PATH" ]; then
  ...
  exec ${QEMU_NAME} /usr/bin/ld `echo "${args[@]}" | sed -e "s#${CROSS_ACCELERATION_DIRECTORY}##"`
fi

CROSS_LD_ARGUMENTS=$@
# Add option "--sysroot=/" if there is no option "--sysroot" in command line.
if [[ ${NATIVE_LD_ARGUMENTS} != "*--sysroot=*" ]] ; then
  CROSS_LD_ARGUMENTS="${NATIVE_LD_ARGUMENTS} --sysroot=/"
fi

NATIVE_LD_ARGUMENTS=$@
# Remove prefix /emul/*-for* from command-line arguments:
NATIVE_LD_ARGUMENTS=`echo ${CROSS_LD_ARGUMENTS} | sed -e "s#${CROSS_ACCELERATION_DIRECTORY}##"`

# Remove option "--sysroot=*" from command-line arguments:
NATIVE_LD_ARGUMENTS=`echo ${CROSS_LD_ARGUMENTS} | sed -e "s#--sysroot=[^[:space:]]\+# #g"`

# Add option -L so that to advise ld where to look for GCC binaries.
NATIVE_LD_ARGUMENTS="${CROSS_LD_ARGUMENTS} -L${GCC_LIBEXEC_DIRECTORY}"
...
NATIVE_LD_NAME="${CROSS_BIN_PREFIX}/${LD_NAME}"
