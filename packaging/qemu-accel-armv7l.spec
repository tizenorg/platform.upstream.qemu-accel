...
for i in "$@"; do
  if [ "${i:0:10}" = "--sysroot=" ]; then
    %{our_path}/usr/arm-tizen-linux-gnueabi/bin/ld.real "$@" || ( /usr/bin/qemu-arm /usr/armv7l-tizen-linux-gnueabi/bin/ld -L/usr/lib/gcc/armv7l-tizen-linux-gnueabi/%{gcc_version_dot}/ `echo "$@" | sed -e "s#%{our_path}##;s#--sysroot=[^[:space:]]\+# #g"` ; echo "Running native ld, because cross ld has failed with the following error: " )
  fi
done

%{our_path}/usr/arm-tizen-linux-gnueabi/bin/ld.real --sysroot=/ "$@" || ( /usr/bin/qemu-arm /usr/armv7l-tizen-linux-gnueabi/bin/ld -L/usr/lib/gcc/armv7l-tizen-linux-gnueabi/%{gcc_version_dot}/ `echo "$@" | sed -e "s#%{our_path}##;s#--sysroot=[^[:space:]]\+# #g"` ; echo "Running native ld, because cross ld has failed with the following error: " )
