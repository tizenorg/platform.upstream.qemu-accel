...
if $sanitizer_enabled; then
   ...
   sed -i -e '/[[:space:]]\+postun "#PLUGIN_POSTUN#"/a  postun "#ASAN_POSTUN#"' %{_sourcedir}/baselibs.conf
   sed -i -e "s,#ASAN_POSTIN#,(ldd %{buildroot}/%{our_path}%{_bindir}/rpm | grep 'libasan') \&\& echo '%{our_path}%{_libdir}/libasan.so' >> /etc/ld.so.preload," %{_sourcedir}/baselibs.conf
