...
do
  ...
  # XXX hack alert! Only works for %{emulated_arch_long}-on-x86_64
%ifarch x86_64
  [ "$(basename $outfile)" = "bash" ] && sed -i 's/x86_64/%{emulated_arch_long}/g' "$outfile"
%endif
...
ln -sf /qemu/qemu-%{emulated_arch_short} %buildroot/usr/bin/qemu-%{emulated_arch_short}
ln -sf /qemu/qemu-%{emulated_arch_short}-binfmt %buildroot/usr/bin/qemu-%{emulated_arch_short}-binfmt

if [[ %{emulated_arch_short} != %{emulated_arch_synonim} ]] ; then
  ln -sf /qemu/qemu-%{emulated_arch_short} %buildroot/usr/bin/qemu-%{emulated_arch_synonim}
  ln -sf /qemu/qemu-%{emulated_arch_short} %buildroot/qemu/qemu-%{emulated_arch_synonim}
  ln -sf /qemu/qemu-%{emulated_arch_short}-binfmt %buildroot/usr/bin/qemu-%{emulated_arch_synonim}-binfmt
